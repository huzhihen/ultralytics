...
"""
YOLOv8 检测训练脚本（仿照 v11 版本）。

详细使用说明：
1) 基础训练（使用预训练权重）：
   python train.py --data coco.yaml --weights yolov8n.pt --epochs 100 --batch-size 16 --imgsz 640
2) 从模型配置文件开始训练：
   python train.py --data coco.yaml --cfg ultralytics/cfg/models/v8/yolov8n.yaml --epochs 100
3) 指定显卡与输出目录：
   python train.py --data custom.yaml --weights yolov8s.pt --device 0 --project runs/train --name exp_v8
4) 断点续训：
...
def main(opt):
    ...
    LOGGER.info(f"Starting YOLOv8 training with arguments: {opt}")

    # Initialize model
    if opt.cfg and opt.weights:
        model = YOLO(opt.cfg).load(opt.weights)
        LOGGER.info(f"Created model from config {opt.cfg} and loaded weights: {opt.weights}")
    elif opt.cfg:
        model = YOLO(opt.cfg)
