...
"""
YOLO11 检测自动标注脚本，支持按 `--data` 中的类别过滤可视化和标注输出，并支持 JSON/TXT 两种格式。

详细使用说明：
1. 对图片目录自动生成检测标注：
   python ultralytics/models/yolo/11/autolabeling_detection.py \
       --weights yolo11n.pt \
...
def parse_opt() -> argparse.Namespace:
    ...
    parser.add_argument("--weights", type=str, default="yolo11n.pt", help="detection model weights path")
