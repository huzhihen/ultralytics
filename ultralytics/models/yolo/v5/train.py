...
详细使用说明：
1. 使用 YOLOv5u 预训练权重训练检测模型：
   python ultralytics/models/yolo/v5/train.py --weights yolov5nu.pt --data coco128.yaml --epochs 100 --img 640
2. 使用模型配置从头训练：
   python ultralytics/models/yolo/v5/train.py --cfg ultralytics/cfg/models/v5/yolov5.yaml --data coco128.yaml
3. 使用 P6 模型配置从头训练：
   python ultralytics/models/yolo/v5/train.py --cfg ultralytics/cfg/models/v5/yolov5-p6.yaml --data coco128.yaml --img 1280
4. 从已有训练结果继续训练：
   python ultralytics/models/yolo/v5/train.py --weights runs/train/exp/weights/best.pt --data coco128.yaml --resume
5. 常用参数：
   --weights     初始权重路径，不传时默认使用 yolov5nu.pt
...
def main(opt):
    """Run YOLOv5 detection training."""
    LOGGER.info(f"Starting YOLOv5 training with arguments: {opt}")
    ...
    if opt.cfg:
        ...
        LOGGER.info("Using default YOLOv5n-u model")
