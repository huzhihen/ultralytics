...
"""
YOLO26 分割模型导出脚本，默认导出 TensorRT engine，也可导出 ONNX、TorchScript 等格式。

详细使用说明：
1. 导出官方分割模型到 TensorRT engine：
   python ultralytics/models/yolo/26/segment/export.py --weights yolo26n-seg.pt --format engine --imgsz 640 --device 0
2. 导出分割模型到 ONNX：
   python ultralytics/models/yolo/26/segment/export.py --weights yolo26n-seg.pt --format onnx --imgsz 640 --dynamic
3. 导出自定义训练权重：
   python ultralytics/models/yolo/26/segment/export.py --weights runs/train-seg/exp/weights/best.pt --format engine
4. 半精度导出：
   python ultralytics/models/yolo/26/segment/export.py --weights yolo26n-seg.pt --format engine --device 0 --half
5. 常用参数：
   --weights   分割模型权重路径
...
def parse_opt():
    ...
    parser.add_argument("--weights", type=str, default="yolo26n-seg.pt", help="model weights path")
...
def main(opt):
    """Run YOLO26 segmentation model export."""
    LOGGER.info(f"Starting YOLO26 segmentation model export with arguments: {opt}")
