...
详细使用说明：
1. 验证官方分割模型：
   python ultralytics/models/yolo/26/segment/val.py --weights yolo26n-seg.pt --data coco128-seg.yaml --img 640
2. 验证训练后的分割权重：
   python ultralytics/models/yolo/26/segment/val.py --weights runs/train-seg/exp/weights/best.pt --data coco128-seg.yaml
3. 指定批大小、设备和输出目录：
   python ultralytics/models/yolo/26/segment/val.py --weights yolo26n-seg.pt --data coco128-seg.yaml \
       --batch-size 32 --device 0 --project runs/val-seg --name yolo26_seg
...
def parse_opt():
    ...
    parser.add_argument("--weights", nargs="+", type=str, default="yolo26n-seg.pt", help="model.pt path(s)")
...
def main(opt):
    """Run YOLO26 segmentation validation."""
    LOGGER.info(f"Starting YOLO26 segmentation validation with arguments: {opt}")
