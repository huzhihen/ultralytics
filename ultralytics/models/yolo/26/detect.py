...
详细使用说明：
1. 摄像头实时检测：
   python ultralytics/models/yolo/26/detect.py --weights yolo26n.pt --source 0
2. 单张图片检测：
   python ultralytics/models/yolo/26/detect.py --weights yolo26n.pt --source img.jpg
3. 视频检测：
   python ultralytics/models/yolo/26/detect.py --weights yolo26n.pt --source video.mp4
4. 文件夹或通配符检测：
   python ultralytics/models/yolo/26/detect.py --weights yolo26n.pt --source path/to/images
   python ultralytics/models/yolo/26/detect.py --weights yolo26n.pt --source "path/to/images/*.jpg"
5. 网络流检测：
   python ultralytics/models/yolo/26/detect.py --weights yolo26n.pt --source "rtsp://example.com/media.mp4"
6. 常用参数：
   --imgsz       推理尺寸，默认 640
...
def parse_opt():
    ...
    parser.add_argument("--weights", nargs="+", type=str, default="yolo26n.pt", help="model path or triton URL")
...
def main(opt):
    """Run YOLO26 detection prediction."""
    LOGGER.info(f"Starting YOLO26 detection with arguments: {opt}")
